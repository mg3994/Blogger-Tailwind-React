...
export class ProductRenderer {
    render(p, state, onVariantChange) {
        ...
        if (isBusiness) {
            ...
            if (variant && p.variesBy) {
                SchemaExtractor.getArray(p.variesBy).forEach((u) => {
                    ...
                    if (variantVal)
                        state.selectedVariants[a] = String(variantVal);
                });
            }
            ...
            const brand = typeof variantBrand === "string"
                ? variantBrand
                : SchemaExtractor.getFirst(variantBrand?.name);
            UIManager.setContent("p-brand", brand || "");
            const offer = SchemaExtractor.getFirst(variant.offers || p.offers);
    ...
    renderBusinessView(b) {
        ...
        const priceEl = UIManager.el("p-price");
        if (priceEl)
            priceEl.textContent = "Service Provider";
    ...
    renderAreaServed(s) {
        ...
        if (s && areaServed && pDesc) {
            const area = typeof areaServed === 'string' ? areaServed : (SchemaExtractor.getFirst(areaServed.name) || SchemaExtractor.getFirst(areaServed['@type']));
    ...
    renderQuantityConstraints(offer) {
        ...
        window.currentQuantityLimits = { minValue, maxValue: effectiveMax };
    ...
    renderCarousel(imgs) {
        ...
        imgs.forEach((s, i) => {
            ...
            if (thumbRow && imgs.length > 1) {
                ...
                t.src = url;
                t.onclick = () => window.AntinnaEngine.goToSlide(i);
    ...
    renderVariants(p, state, onVariantChange) {
        ...
        if (vc && !vc.children.length) {
            if (p.variesBy) {
                ...
                SchemaExtractor.getArray(p.variesBy).forEach((u) => {
                    ...
                    const vals = [...new Set(variants.map((x) => SchemaExtractor.getFirst(x[a])).filter(Boolean))];
                    if (vals.length === 0)
                        return;
                    ...
                    vals.forEach((vl) => {
                        ...
                        if (a.toLowerCase() === "color") {
                            ...
                            const variants = SchemaExtractor.getArray(p.hasVariant);
                            const vm = variants.find((x) => String(SchemaExtractor.getFirst(x[a])) === String(vl));
                    ...
                    g.appendChild(os);
                    vc.appendChild(g);
                    if (!state.selectedVariants[a])
                        state.selectedVariants[a] = String(vals[0]);
                });
            }
                ...
                SchemaExtractor.getArray(catalog?.itemListElement).forEach((off) => {
                    const btn = document.createElement("button");
                    btn.className = "v-btn";
    ...
    checkAvailability(p, state) {
        ...
        document.querySelectorAll('.v-btn[data-attr]').forEach(btn => {
            ...
            const match = variants.find((x) => Object.entries(test).every(([k, val]) => {
    ...
    renderSpecs(variant, p) {
        ...
        if (sp && sl) {
            ...
            if (audience) {
                ...
                if (audienceName)
                    flds['Audience'] = audienceName;
                if (suggestedAge)
                    flds['Suggested Age'] = suggestedAge;
            }
            // Certifications
            ...
            for (let [l, k] of Object.entries(flds)) {
                if (k)
                    h += `<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid rgba(0,0,0,0.05);"><span style="opacity:0.6;">${l}</span><span style="font-weight:700;">${k}</span></div>`;
            }
    ...
    renderSeller(s) {
        ...
        const maps = UIManager.el("maps-link");
        if (!box || !inf)
            return;
        if (!s) {
    ...
    renderOtherServices(s, p) {
        ...
        if (services.length > 0) {
            ...
            otherList.innerHTML = services.map((ser) => {
    ...
    renderAddOns(addOns, s, p) {
        ...
        const parentKey = cartManager.generateItemKey({ ...p, url: currentUrl }, window.AntinnaEngine.state.selectedVariants);
        const isParentInCart = cartManager.getOrder().orderedItem.some((oi) => oi.itemKey === parentKey);
        list.innerHTML = addOns.map((ser) => {
            const rawItem = SchemaExtractor.getFirst(ser.itemOffered) || ser;
            const n = SchemaExtractor.getFirst(rawItem.name) || SchemaExtractor.getFirst(ser.name);
            const { price, currency } = SchemaExtractor.extractPrice(ser);
            const symbol = SchemaExtractor.getCurrencySymbol(currency);
