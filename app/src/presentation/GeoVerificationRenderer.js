...
export class GeoVerificationRenderer {
    ...
    targetMarker;
    debounceTimer;
    appsScriptService = AppsScriptService.getInstance();
    currentDeviceLat = 28.52785;
    currentDeviceLng = 76.08361;
    isAddressModified = false;
    constructor(locationManager) {
        const loc = locationManager.getData();
        if (loc.lat)
            this.currentDeviceLat = loc.lat;
        if (loc.lon)
            this.currentDeviceLng = loc.lon;
    }
    renderPopup() {
    ...
    setupListeners() {
        const input = UIManager.el('antinna-geo-search');
        if (input) {
            input.oninput = (e) => this.handleTypeAhead(e.target.value);
        }
        ...
        if (finalizeBtn) {
            finalizeBtn.onclick = async () => {
                try {
                    ...
                    window.AntinnaEngine.setOrderDelivery(deliveryData);
                    this.setFinalizeLoading(false);
                    window.AntinnaEngine.showOrderSummary();
                }
                catch (e) {
    ...
    validateAddressForm() {
        const extended = UIManager.el('geo-extendedAddress')?.value.trim();
        const street = UIManager.el('geo-streetAddress')?.value.trim();
        const pin = UIManager.el('geo-postalCode')?.value.trim();
        const finalizeBtn = UIManager.el('antinna-geo-finalize-btn');
        if (finalizeBtn) {
    ...
    collectDeliveryData() {
        ...
        const lat = pos ? pos.lat : (window.lastGeoResponse?.lat || 0);
        const lng = pos ? pos.lng : (window.lastGeoResponse?.lng || 0);
        return {
            "@type": "ParcelDelivery",
            "deliveryName": "Standard Handheld Delivery",
            "deliveryAddress": {
                "@type": "PostalAddress",
                "extendedAddress": UIManager.el('geo-extendedAddress')?.value,
                "streetAddress": UIManager.el('geo-streetAddress')?.value,
                "addressLocality": UIManager.el('geo-locality')?.value,
                "addressRegion": window.lastGeoResponse?.addressDetails?.addressRegion || "HR",
                "postalCode": UIManager.el('geo-postalCode')?.value,
                "addressCountry": "IN"
            },
            "deliveryStatus": {
                "@type": "DeliveryEvent",
    ...
    async initMap() {
        ...
        if (!L)
            return;
        const center = [this.currentDeviceLat, this.currentDeviceLng];
        if (!this.map) {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(pos => {
                ...
                const loc = [this.currentDeviceLat, this.currentDeviceLng];
    ...
    async handleManualPinPosition(lat, lng) {
        ...
        suggestions.forEach(text => {
            ...
            item.onclick = async () => {
                ...
                try {
                    ...
                    if (response.success) {
                        const newPos = [response.lat, response.lng];
    ...
    updateTelemetryUI(response) {
        if (response.success) {
            window.lastGeoResponse = response;
            ...
            if (form) {
                ...
                if (response.addressDetails) {
                    ...
                    const extInput = UIManager.el('geo-extendedAddress');
                    const streetInput = UIManager.el('geo-streetAddress');
                    if (!this.isAddressModified) {
                        extInput.value = d.extendedAddress || "";
                        streetInput.value = d.streetAddress || "";
                    }
                    UIManager.el('geo-locality').value = d.addressLocality || "";
                    UIManager.el('geo-postalCode').value = d.postalCode || "";
                }
                this.validateAddressForm();
            }
            // Save verified location to state
            window.AntinnaEngine.setVerifiedLocation(response);
        }
    }
}
