...
export class OrderSummaryRenderer {
    ...
    render(verifiedLocation, orderDelivery) {
        ...
        const itemsHtml = SchemaExtractor.getArray(order.orderedItem).map((item) => {
            ...
            // Render Addons
            const addOnsHtml = SchemaExtractor.getArray(item.addOns).map((addon) => {
    ...
    renderGooglePayButton(order, verifiedLocation, isServiceable) {
      ...
      `;
        ...
        btn.onclick = () => {
            window.GooglePayService.initPayment(order, verifiedLocation);
        };
