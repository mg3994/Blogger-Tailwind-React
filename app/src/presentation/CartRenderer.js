...
export class CartRenderer {
    ...
    renderFab() {
        ...
        if (!fab) {
            ...
            fab.onclick = () => window.AntinnaEngine.refreshCartData();
    ...
    updateUI() {
        ...
        const proceedBtn = UIManager.el("cart-proceed-btn");
        if (proceedBtn) {
    ...
    showModal() {
        ...
        list.innerHTML = orderedItems.map((item, idx) => {
            ...
            let statusText = '';
            if (isUnavailable)
                statusText = '<div style="color:red; font-size:0.7rem; font-weight:800;">Currently Unavailable</div>';
            else if (isOutOfStock)
                statusText = '<div style="color:orange; font-size:0.7rem; font-weight:800;">Out of Stock</div>';
            else if (!isQuantityValid)
                statusText = `<div style="color:#ef4444; font-size:0.7rem; font-weight:800;">Minimum ${item._constraints?.minValue} required</div>`;
            ...
            // Render Addons
            const addOnsHtml = SchemaExtractor.getArray(item.addOns).map((addon, aIdx) => {
      ...
      `;
        ...
        if (totalEl)
            totalEl.textContent = `${SchemaExtractor.getCurrencySymbol(order.priceCurrency || 'INR')}${order.totalPrice || 0}`;
    ...
    getItemImage(item) {
        if (Array.isArray(item.image))
            return item.image[0]?.url || item.image[0] || '';
