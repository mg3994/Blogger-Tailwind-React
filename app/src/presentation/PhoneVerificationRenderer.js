...
export class PhoneVerificationRenderer {
    ...
    resendTimer = null;
    countdown = 60;
    selectedCountry = { code: '+91', name: 'India', flag: '🇮🇳' };
    countries = [
    ...
    setupListeners() {
        ...
        if (sendBtn)
            sendBtn.onclick = () => this.handleSendOTP();
        if (verifyBtn)
            verifyBtn.onclick = () => this.handleVerifyOTP();
        if (resendBtn)
            resendBtn.onclick = () => this.handleSendOTP();
        if (trigger) {
            trigger.onclick = (e) => {
                e.stopPropagation();
    ...
    initRecaptcha() {
        ...
        if (!window.recaptchaVerifier) {
            try {
                ...
                const { RecaptchaVerifier } = window.firebaseAuthInternal || {};
    ...
    startResendTimer() {
        ...
        const btn = UIManager.el('antinna-resend-btn');
        if (btn)
            console.log("Resend btn ready");
        const countEl = UIManager.el('antinna-countdown');
        this.resendTimer = setInterval(() => {
            this.countdown--;
            if (countEl)
                countEl.textContent = String(this.countdown);
            if (this.countdown <= 0) {
                clearInterval(this.resendTimer);
                ...
                if (container) {
                    ...
                    if (newBtn)
                        newBtn.onclick = () => this.handleSendOTP();
                }
            }
        }, 1000);
    }
    async handleSendOTP() {
        const phoneInput = UIManager.el('antinna-phone-number');
        ...
        const linkWithPhoneNumber = window.linkWithPhoneNumber;
        ...
        try {
            ...
            this.confirmationResult = await linkWithPhoneNumber(user, phone, window.recaptchaVerifier);
    ...
    async handleVerifyOTP() {
        const otpInput = UIManager.el('antinna-otp-code');
        ...
        try {
            ...
            window.AntinnaEngine.showGeoVerification();
        }
        catch (error) {
