export class UIManager {
    ...
    static setContent(idOrSelector, content) {
        const e = this.el(idOrSelector) || this.query(idOrSelector);
        if (e)
            e.textContent = content;
    }
    static setHtml(idOrSelector, html) {
        const e = this.el(idOrSelector) || this.query(idOrSelector);
        if (e)
            e.innerHTML = html;
    }
    static toggleClass(idOrSelector, className, force) {
        const e = this.el(idOrSelector) || this.query(idOrSelector);
        if (e)
            e.classList.toggle(className, force);
    }
    static injectModalStyles() {
        if (document.getElementById('antinna-modal-styles'))
            return;
    ...
    static hide3DViewer() {
        ...
        if (backdrop) {
            ...
            const container = this.el('antinna-3d-container');
            if (container)
                container.innerHTML = ''; // Stop the model viewer
        }
    }
