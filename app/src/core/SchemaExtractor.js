export class SchemaExtractor {
    ...
    static findMatchingVariant(parent, selectedAttributes, lastClickedAttr = null) {
        if (!parent)
            return null;
        const variants = this.getArray(parent.hasVariant).length > 0 ? this.getArray(parent.hasVariant) : [parent];
        let match = variants.find((v) => Object.entries(selectedAttributes).every(([k, val]) => String(this.getFirst(v[k])) === String(val)));
        if (!match && lastClickedAttr) {
            match = variants.find((v) => String(this.getFirst(v[lastClickedAttr])) === String(selectedAttributes[lastClickedAttr]));
        }
    ...
    static findMatchingServicePackage(parent, packageName) {
    ...
    static extractPriceForQuantity(offer, quantity) {
        ...
        if (specs.length > 0) {
            const matchingSpec = specs.find((spec) => {
                if (this.getFirst(spec["@type"]) !== "UnitPriceSpecification")
                    return false;
                const eq = this.getFirst(spec.eligibleQuantity);
                if (!eq)
                    return true; // Default if no quantity specified on this spec
    ...
    static extractAvailability(offer) {
        const off = Array.isArray(offer) ? offer[0] : offer;
        if (!off)
        ...
        return av?.["@id"] || String(av);
    }
    static extractEligibleQuantity(data) {
    ...
    static extractInventoryLevel(data) {
        const obj = Array.isArray(data) ? data[0] : data;
        if (!obj)
            return null;
    ...
    static extractDimensions(data) {
        const obj = Array.isArray(data) ? data[0] : data;
        if (!obj)
            return { weight: null, height: null, width: null, depth: null };
        const getNum = (v) => {
            if (v === undefined || v === null)
                return null;
            if (typeof v === 'object')
                return Number(this.getFirst(v.value)) || null;
    ...
    static extractAdvanceBookingRequirement(offer) {
        const off = Array.isArray(offer) ? offer[0] : offer;
        if (!off)
            return null;
    ...
    static extractCondition(data) {
        ...
        if (str.includes("newcondition"))
            return "New";
        if (str.includes("refurbishedcondition"))
            return "Refurbished";
        if (str.includes("usedcondition"))
            return "Used";
        if (str.includes("damagedcondition"))
            return "Damaged";
        return str.split('/').pop() || str;
    }
    static extractAreaServed(data) {
        const results = [];
        const stack = [data];
        const seen = new Set();
        while (stack.length > 0) {
            ...
            if (current.itemOffered)
                stack.push(current.itemOffered);
            if (current.offers)
                stack.push(...this.getArray(current.offers));
            if (current.hasVariant)
                stack.push(...this.getArray(current.hasVariant));
            if (current.hasOfferCatalog)
                stack.push(...this.getArray(current.hasOfferCatalog));
            if (current.itemListElement)
                stack.push(...this.getArray(current.itemListElement));
        }
        return results;
    }
    static isBusinessOpen(data) {
        const obj = Array.isArray(data) ? data[0] : data;
        if (!obj)
        ...
        for (const s of special) {
            ...
            if (from && through && todayStr >= String(from) && todayStr <= String(through)) {
                ...
                if (opens === "00:00" && closes === "00:00")
                    return { isOpen: false, message: "Closed for Holiday/Event" };
    ...
    static extractLeadTime(data) {
        ...
        if (typeof lt === 'object') {
            ...
            if (unit === 'HUR' || unit === 'hour' || unit === 'hours')
                return val * 60;
            if (unit === 'DAY' || unit === 'day' || unit === 'days')
                return val * 24 * 60;
    ...
    static isLocationInArea(targetLat, targetLon, targetAddress, area) {
        if (!area)
            return true; // If no area defined, assume global
        ...
        if (type === 'City' || type === 'AdministrativeArea' || type === 'State' || type === 'Country') {
            ...
            if (name === tCity || name === tState || name === tCountry)
                return true;
        }
